...
const CLUE_DATA = {
  ...
  Dn9: {
    clue:
      "הוא נראה בלי זנבו כשנישא מסביב וזה היה התפקיד של גולדה [ראשי תיבות] (4)",
    answer: "רוהמ",
    direction: "down",
    number: 9,
    boxes: ["C5", "D5", "E5", "F5"],
  ...
  handleFocus(event) {
    ...
    if (dir === "A" || dir === "D") {
      ...
      boxesToHighlight = boxesToHighlight.concat(
        this.props.allClues[this.props.boxClues[0]].boxes
      );
    }
  }
  ...
  render() {
    ...
    function handleMovement(props, e) {
      ...
      if (e.keyCode !== 8 && e.type === "keydown") {
        ...
        let indexCell = Number(
          arrayRange.indexOf(e.target.id.split("input")[1])
        );
        setTimeout(() => {
          const elementToFocus = document.getElementById(
            `input${arrayRange[indexCell + 1]}`
          );
          if (elementToFocus) {
    ...
    function handleClick(props, e) {
      ...
      props.setActiveClueBoxes(
        sessionStorage.getItem("movementRange").split(",")
      );
